...
def max_pool(name,x,k):
    with tf.name_scope(name), tf.variable_scope(name):
        max_pool = tf.nn.max_pool(x,ksize=[1,k,k,1],strides=[1,k,k,1],padding='SAME',name='max_pool')
    return max_pool

def fc(input,units=1024,activation=tf.nn.relu):

    logits = tf.layers.dense(inputs=input,
                             units=units,
                             activation=activation,
                             kernel_initializer=tf.truncated_normal_initializer(stddev=0.05, mean=0.0),
...
if __name__ == '__main__':
    ...
    a = tf.ones(shape=[2,20])
    fc = fc(input=a,units=512)
    print(fc.get_shape())
